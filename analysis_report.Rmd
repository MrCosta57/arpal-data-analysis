...
requirements <- c("dplyr", "ggplot2", "kableExtra", "ARPALData", "imputeTS", "xts", "forecast")
...
To simplify visualization and focus on recent data, the time series for this part of the study is limited to the period from January 1, 2022, to December 31, 2023. As shown previously, the choice of selecting a time window at the end of the COVID-19 emergency period in Italy don't influence much the PM10 values pattern
...
**Station 548 - Milano v.Senato**
```{r}
ts_m_train_test <- window_ts_xts(ts_m, df_aq_m$Date, train_set_start, test_set_end)
ts_m_train <- window_ts_xts(ts_m, df_aq_m$Date, train_set_start, train_set_end)
ts_m_test <- window_ts_xts(ts_m, df_aq_m$Date, test_set_start, test_set_end)
...
plot_acf_pacf(ts_m_train, aq_station_names_code[1], lag_max = 40)
plot_log_arima_acf_pacf(ts_m_train, aq_station_names_code[1], lag_max = 40)
```

Considering the plots can be clearly observed that time series is not stationary. In particular by cheching the ACF plot the lags decay very slow. 
Also if logarithmic transformation is applied to time series for stabilizing the variance, the situation is more or less invariate.
The lag is show until 40 lags, because after more than a month of inspection of daily correlation the lags can be not significant.

```{r}
plot_diff_arima_acf_pacf(ts_m_train, aq_station_names_code[1], lag_max = 40)
plot_diff_arima_acf_pacf(ts_m_train, aq_station_names_code[1], lag_max = 40, lag = 7)
```

Differencing time series instead seems to be very benefician indeed data seems now more alliged to stationary assumption.
Differencing by lag 7 instead seems to be useless and introduce some forced seasonal pattern to data. 
TODO: MODIFY Nevertheless, applying differencing to eliminate potential periodic factors might be impractical and risky in the case of daily observations. This is because we are not considering time domains such as months or weeks where a certain type of pattern is expected over a relatively large time period. The difference between days in two different years is highly variable, influenced by factors like weather or the day of the week, making it uninformative and challenging to utilize effectively.
So difficult to see or predict seasonality

Given that the differenced time series ACF plot show some more evident spikes until lag 5 and PACF display relatively fast decay, an ARIMA model with order (0, 1, 5) is fitted. The residuals are checked to ensure they are white noise, and the model is compared with an automatic ARIMA model selection.
...
```{r}
ic_ts_m <- get_arima_IC(arima_models = list(ma5_ts_m, auto_ts_m), suffixes = list("", "[auto]"))
print(ic_ts_m)
```

TODO: Describe results
The same procedure is applied also for the other stations, by differencing one time the time series to make them more stationary and inspecting the plot for guess the best ARIMA model for them.
In the end also for the others stations are compared with automatic ARIMA model selection. In both cases in the other stations residuals seems to be coherent with the assumptions

**Station 571 - Bormio v.Monte Braulio**
```{r}
ts_b_train_test <- window_ts_xts(ts_b, df_aq_b$Date, train_set_start, test_set_end)
ts_b_train <- window_ts_xts(ts_b, df_aq_b$Date, train_set_start, train_set_end)
ts_b_test <- window_ts_xts(ts_b, df_aq_b$Date, test_set_start, test_set_end)
...
plot_acf_pacf(ts_b_train, aq_station_names_code[2], lag_max = 40)
```
```{r}
plot_diff_arima_acf_pacf(ts_b_train, aq_station_names_code[2], lag_max = 40)
```
```{r}
...
```{r}
ic_ts_b <- get_arima_IC(arima_models = list(ar1_ma2_ts_b, auto_ts_b), suffixes = list("", "[auto]"))
print(ic_ts_b)
```

**Station 703 - Schivenoglia v. Malpasso**
```{r}
ts_s_train_test <- window_ts_xts(ts_s, df_aq_s$Date, train_set_start, test_set_end)
ts_s_train <- window_ts_xts(ts_s, df_aq_s$Date, train_set_start, train_set_end)
ts_s_test <- window_ts_xts(ts_s, df_aq_s$Date, test_set_start, test_set_end)
...
plot_acf_pacf(ts_s_train, aq_station_names_code[3], lag_max = 40)
```
```{r}
plot_diff_arima_acf_pacf(ts_s_train, aq_station_names_code[3], lag_max = 40)
```
```{r}
...
```{r}
ic_ts_s <- get_arima_IC(arima_models = list(ar1_ma3_ts_s, auto_ts_s), suffixes = list("", "[auto]"))
print(ic_ts_s)
```

In all the stations model fitted using automatic criteria selection have lower value of AICc so these one are selected.
Notice that the manual selected model in some stations (e.i station 548 and 571) have lower Bic value so they appear better is a more parsimonious model is desidered.

## Dynamic regression

NO2, N0x and PM10 have the same unit of measure
CO has a different unit of measure

Nitrogen Dioxide (NO2): NO2 is a significant precursor to particulate matter formation. It can react in the atmosphere to form secondary particles, contributing to PM10 levels.
Nitrogen Oxides (NOx): Similar to NO2, NOx (which includes both NO and NO2) plays a crucial role in the formation of secondary particulate matter. These compounds are primarily emitted from combustion processes, such as those in vehicles and industrial facilities, and are directly linked to PM10 concentrations.
Carbon Monoxide (CO): While CO is primarily a gas, it can contribute indirectly to PM10 levels through its role in atmospheric chemical reactions that lead to the formation of secondary pollutants, including particulate matter.
Benzene, while a hazardous air pollutant, is less directly related to PM10 formation compared to the other three pollutants.

```{r}
no2_ts_m <- xts(df_aq_m$NO2, order.by = df_aq_m$Date)
no2_ts_b <- xts(df_aq_b$NO2, order.by = df_aq_b$Date)
no2_ts_s <- xts(df_aq_s$NO2, order.by = df_aq_s$Date)
no2_ts_m_train <- window_ts_xts(no2_ts_m, df_aq_m$Date, train_set_start, train_set_end)
no2_ts_b_train <- window_ts_xts(no2_ts_b, df_aq_b$Date, train_set_start, train_set_end)
no2_ts_s_train <- window_ts_xts(no2_ts_s, df_aq_s$Date, train_set_start, train_set_end)
no2_ts_m_train <- xts2ts(imputeTS::na_interpolation(no2_ts_m_train), daily_ts_freq)
no2_ts_b_train <- xts2ts(imputeTS::na_interpolation(no2_ts_b_train), daily_ts_freq)
no2_ts_s_train <- xts2ts(imputeTS::na_interpolation(no2_ts_s_train), daily_ts_freq)

# co_ts_m <- xts(df_aq_m$CO, order.by = df_aq_m$Date)
# co_ts_b <- xts(df_aq_b$CO, order.by = df_aq_b$Date)
# co_ts_s <- xts(df_aq_s$CO, order.by = df_aq_s$Date)
# co_ts_m_train <- window_ts_xts(co_ts_m, df_aq_m$Date, train_set_start, train_set_end)
# co_ts_b_train <- window_ts_xts(co_ts_b, df_aq_b$Date, train_set_start, train_set_end)
# co_ts_s_train <- window_ts_xts(co_ts_s, df_aq_s$Date, train_set_start, train_set_end)
## Convert milligrams per cubic meter to micrograms per cubic meter
# co_ts_m_train <- xts2ts(imputeTS::na_interpolation(co_ts_m_train), daily_ts_freq) * 1000
# co_ts_b_train <- xts2ts(imputeTS::na_interpolation(co_ts_b_train), daily_ts_freq) * 1000
# co_ts_s_train <- xts2ts(imputeTS::na_interpolation(co_ts_s_train), daily_ts_freq) * 1000

# nox_ts_m <- xts(df_aq_m$NOx, order.by = df_aq_m$Date)
# nox_ts_b <- xts(df_aq_b$NOx, order.by = df_aq_b$Date)
# nox_ts_s <- xts(df_aq_s$NOx, order.by = df_aq_s$Date)
# nox_ts_m_train <- window_ts_xts(nox_ts_m, df_aq_m$Date, train_set_start, train_set_end)
# nox_ts_b_train <- window_ts_xts(nox_ts_b, df_aq_b$Date, train_set_start, train_set_end)
# nox_ts_s_train <- window_ts_xts(nox_ts_s, df_aq_s$Date, train_set_start, train_set_end)
# nox_ts_m_train <- xts2ts(imputeTS::na_interpolation(nox_ts_m_train), daily_ts_freq)
# nox_ts_b_train <- xts2ts(imputeTS::na_interpolation(nox_ts_b_train), daily_ts_freq)
# nox_ts_s_train <- xts2ts(imputeTS::na_interpolation(nox_ts_s_train), daily_ts_freq)
```

It seems that between NO2 and PM10 there's a linear relationship. This is confirmed by the linear regression model where the predictor appear strongly significant. 
The residuals however are clearly violating the white noise assumption. The same procedure is applied to the other stations and the results are very similar.
```{r}
...
By analyzing the relation between PM10 and CO and PM10 and NOx the situation is similar.
```{r}
# plot_pollutant_XY_lin(
#  x = co_ts_m_train,
#  y = ts_m_train,
...
It's decided to fit a dynamic regression model to the time series data, considering just NO2 because it seems to be the most related predictor.
The model is estimated using the auto.arima function, which automatically selects the best ARIMA model based on the corrected Akaike Information Criterion (AICc). The residuals are then checked for white noise using the checkresiduals function.
...
A non linear model is tried to fit the data. The model is a neural network autoregressive model, which is a feedforward neural network with a single hidden layer. The model is estimated using the nnetar function, which automatically selects the best neural network model based on the Akaike Information Criterion (AIC).
In particular, for non-seasonal data the fitted model is denoted as an $NNAR(p,k)$ model, where k is the number of hidden nodes. This is analogous to an AR(p) model but with nonlinear functions. For seasonal data, the fitted model is called an $NNAR(p,P,k)[m]$ model, which is analogous to an $ARIMA(p,0,0)(P,0,0)[m]$ model but with nonlinear function.
Due to *Universal Approximation Theorem* ^[G. Cybenko. "Approximation by superpositions of a sigmoidal function". In:
Mathematics of Control, Signals and Systems 2 (1989), pp. 303-314] a neural network with a single hidden layer can approximate any continuous function on a compact subset.

For non linear model classical residual checking using autocorrelation function may not be sufficient to assess the model correctness. In this case, other types of correlation are checked.
```{r}
nn_ts_m_auto <- nnetar(ts_m_train)
plot_nn_residuals(nn_ts_m_auto, aq_station_names_code[1], ts_m_train)
```

By the ACF plots of residuals and the residual squared seems to be alliged to white noise assumption. Also the Ljung-Box test p-values suggest that there's no wvidence to reject the null hypotesis of white noise.
The cross correlation function plots instead shows that there are probably some information left because some lags appear to be correlated.

The same procedure is applied to the other stations and the results are very similar to the Milan's station.
```{r}
nn_ts_b_auto <- nnetar(ts_b_train)
# plot_nn_residuals(nn_ts_b_auto, aq_station_names_code[2], ts_b_train)
```

```{r}
nn_ts_s_auto <- nnetar(ts_s_train)
# plot_nn_residuals(nn_ts_s_auto, aq_station_names_code[3], ts_s_train)
```
...
yhat <- forecast(auto_ts_m, h = test_set_size, level = c(80, 95))
plot(yhat)
# ts_m_test<- xts2ts(ts_m_test, daily_ts_freq)

auto_ts_m %>%
  forecast(h = test_set_size - 5) %>%
  accuracy(xts2ts(window_ts_xts(ts_m, df_aq_m$Date, train_set_start, test_set_end - 5), daily_ts_freq))

# training <- window(co2.ts, end=c(2012,12)) #split the data in two subsets using `windows` for considering a portion of the TS
# test <- window(co2.ts, start=c(2013,1))
#
# co2.train <- Arima(training, order=c(0,1,1), #ARIMA(0,1,1)(0,1,1)[12]
#                   seasonal=c(0,1,1)) #-> (1-B)(1-B^12)Y_t=(1+theta_1*B)(1+THETA_1*B^12)Z_t
## IT'S A FAMOUS COMMON MODEL USED WHEN DATA ARE SEASONAL
## When I pass order to the `Arima`, function estimate also the parameters
#
# plot(forecast(co2.train,h=48)) #blue value in the plot are the forecasted value
# lines(test,col=2) #red value are the true value
#
#
# co2.test <- Arima(test,model=co2.train) ##When I pass model to the `Arima`, function use that model, don't estimate anything
#
# accuracy(co2.train) # ACF1 is the val of autocorr function of 1 step error, I want that model don't contains autocorrelation in 1 step error
#                    #(a model with autocorrelation on 1 step error don't respect process behavior)
# accuracy(co2.test)
```
