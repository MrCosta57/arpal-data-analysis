...
```{r}
ic_ts_m <- compute_arima_IC(arima_models = list(ma5_ts_m, auto_ts_m), suffixes = list("", "[auto]"))
print(ic_ts_m)
```
...
```{r}
ic_ts_b <- compute_arima_IC(arima_models = list(ar1_ma2_ts_b, auto_ts_b), suffixes = list("", "[auto]"))
print(ic_ts_b)
```
...
```{r}
ic_ts_s <- compute_arima_IC(arima_models = list(ar1_ma3_ts_s, auto_ts_s), suffixes = list("", "[auto]"))
print(ic_ts_s)
```
...
# plot_forecast <- function(model, h, ts, date_range, date_start_train, date_start_test, freq)
# forecast_residuals_analysis <- function(res, model_name)
# compute_forecast_metrics <- function(model, h, ts, date_range, date_start_train, date_start_test, freq)

plot_forecast(auto_ts_m, 31, ts_m_train, ts_m_test, "PM10", "ARIMA")
forecast_residuals_analysis(auto_ts_m, 31, "ARIMA")
compute_forecast_metrics(auto_ts_m, 31, ts_m_train, ts_m_test)
