...
aq_station_names <- c("Station 548 - Milano v.Senato", "Station 571 - Bormio v.Monte Braulio", "Station 703 - Schivenoglia v. Malpasso")
...
#    as.data.frame(t(gap_df_list[i][[1]])),
...
print_table_custom(
  df_aq_stations %>%
    ...
    distinct() %>%
    arrange(Pollutant),
  title = paste(aq_station_names[1], "information")
)
```
```{r}
...
print_table_custom(
  ...
  is_summary = TRUE, title = paste(aq_station_names[1], "daily data statistics"),
...
print_table_custom(df_aq_stations %>%
  ...
  distinct() %>%
  arrange(Pollutant), title = paste(aq_station_names[2], "information"))
```
```{r}
...
print_table_custom(
  df_aq_b %>%
    select(c(-IDStation, -NameStation, -Date)),
  is_summary = TRUE, title = paste(aq_station_names[2], "daily data statistics"),
  highlight_rows = com_poll_names
)
```
...
print_table_custom(df_aq_stations %>%
  ...
  distinct() %>%
  arrange(Pollutant), title = paste(aq_station_names[3], "information"))
```
```{r}
...
print_table_custom(
  df_aq_s %>%
    select(c(-IDStation, -NameStation, -Date)),
  is_summary = TRUE, title = paste(aq_station_names[3], "daily data statistics"),
  highlight_rows = com_poll_names
)
```
...
## Time-related data
imputeTS^[Moritz S, Bartz-Beielstein T (2017). "imputeTS: Time Series Missing Value Imputation in R." The R Journal, 9(1), 207–218. doi:10.32614/RJ-2017-009.]^ package is used to impute missing values in the time series data.
```{r}
...
plot_ts_grid(
  ...
  n_row = 3,
  ts_names = aq_station_names
)
```

```{r}
agg_df_aq_s <- ARPALData::Time_aggregate(
  df_aq_s, "monthly", Var_vec="PM10", Fns_vec=c("mean")
)
agg_df_aq_b <- ARPALData::Time_aggregate(
  df_aq_b, "monthly", Var_vec="PM10", Fns_vec="mean"
)
agg_df_aq_m <- ARPALData::Time_aggregate(
  df_aq_m, "monthly", Var_vec="PM10", Fns_vec="mean"
)
