...
requirements <- c("dplyr", "ggplot2", "kableExtra", "ARPALData", "imputeTS", "xts", "forecast", "TSA", "tsbox")
...
aq_station_names_short <- c("Station 548", "Station 571", "Station 703")
...
plot_ts_grid(
  ts_list = list(
    xts(agg_df_aq_m$PM10, order.by = agg_df_aq_m$Date),
    xts(agg_df_aq_b$PM10, order.by = agg_df_aq_b$Date),
    xts(agg_df_aq_s$PM10, order.by = agg_df_aq_s$Date)
  ),
  ylab = paste("PM10 ", "(", pollutant_units[["PM10"]], ")", sep = ""),
  ts_colors = station_colors,
  n_row = 3,
...
monthplot(
  tsbox::ts_ts(xts(agg_df_aq_m$PM10, order.by = agg_df_aq_m$Date, frequency = 12)),
  main = "Monthly plot of PM10 for Milano v.Senato",
  ylab = paste("PM10 ", "(", pollutant_units[["PM10"]], ")", sep = "")
)
monthplot(
  tsbox::ts_ts(xts(agg_df_aq_b$PM10, order.by = agg_df_aq_b$Date, frequency = 12)),
  main = "Monthly plot of PM10 for Bormio v.Monte Braulio",
  ylab = paste("PM10 ", "(", pollutant_units[["PM10"]], ")", sep = "")
)
monthplot(
  tsbox::ts_ts(xts(agg_df_aq_s$PM10, order.by = agg_df_aq_s$Date, frequency = 12)),
  main = "Monthly plot of PM10 for Schivenoglia v. Malpasso",
  ylab = paste("PM10 ", "(", pollutant_units[["PM10"]], ")", sep = "")
)
```
...
rownames(stats_na_table) <- aq_station_names_short
stats_na_table
```

```{r fig.height=4, fig.width=10}
imputeTS::ggplot_na_distribution(ts_m,
  x_axis_labels = time(ts_m),
  title = paste(aq_station_names_short[1], "-", "missing values"),
  ylab = paste("PM10 ", "(", pollutant_units[["PM10"]], ")", sep = ""),
  color_lines = station_colors[1],
  size_points = NA
)
imputeTS::ggplot_na_distribution2(ts_m,
  title = paste(aq_station_names_short[1], "-", "missing values ratio per interval"),
)
```
```{r fig.height=4, fig.width=10}
imputeTS::ggplot_na_distribution(ts_b,
  x_axis_labels = time(ts_b),
  title = paste(aq_station_names_short[2], "-", "missing values"),
  ylab = paste("PM10 ", "(", pollutant_units[["PM10"]], ")", sep = ""),
  color_lines = station_colors[2],
  size_points = NA
)
imputeTS::ggplot_na_distribution2(ts_b,
  title = paste(aq_station_names_short[2], "-", "missing values ratio per interval"),
)
```
```{r fig.height=4, fig.width=10}
imputeTS::ggplot_na_distribution(ts_s,
  x_axis_labels = time(ts_s),
  title = paste(aq_station_names_short[3], "-", "missing values"),
  ylab = paste("PM10 ", "(", pollutant_units[["PM10"]], ")", sep = ""),
  color_lines = station_colors[3],
  size_points = NA
)
imputeTS::ggplot_na_distribution2(ts_s,
  title = paste(aq_station_names_short[3], "-", "missing values ratio per interval"),
)
```

```{r fig.height=4, fig.width=8}
ts_m_imputed <- imputeTS::na_kalman(ts_m)
ts_b_imputed <- imputeTS::na_kalman(ts_b)
ts_s_imputed <- imputeTS::na_kalman(ts_s)

imputeTS::ggplot_na_imputations(
  window(ts_m, start = "2021-01-01", end = "2022-12-31"),
  window(ts_m_imputed, start = "2021-01-01", end = "2022-12-31"),
  title = paste(aq_station_names_short[1], "-", "Kalman imputation"),
  x_axis_labels = time(window(ts_m, start = "2021-01-01", end = "2022-12-31")),
...
imputeTS::ggplot_na_imputations(
  window(ts_b, start = "2014-01-01", end = "2015-12-31"),
  window(ts_b_imputed, start = "2014-01-01", end = "2015-12-31"),
  title = paste(aq_station_names_short[2], "-", "Kalman imputation"),
  x_axis_labels = time(window(ts_b, start = "2014-01-01", end = "2015-12-31")),
...
imputeTS::ggplot_na_imputations(
  window(ts_s, start = "2016-01-01", end = "2017-12-31"),
  window(ts_s_imputed, start = "2016-01-01", end = "2017-12-31"),
  title = paste(aq_station_names_short[3], "-", "Kalman imputation"),
  x_axis_labels = time(window(ts_s, start = "2016-01-01", end = "2017-12-31")),
...
imputeTS::ggplot_na_imputations(
  tmp_ts_m, ts_m,
  title = paste(aq_station_names_short[1], "-", "outliers detection"),
  x_axis_labels = time(ts_m),
  color_lines = station_colors[1],
  color_imputations = "red",
  ylab = paste("PM10 ", "(", pollutant_units[["PM10"]], ")", sep = ""),
...
imputeTS::ggplot_na_imputations(
  tmp_ts_b, ts_b,
  title = paste(aq_station_names_short[2], "-", "outliers detection"),
  x_axis_labels = time(ts_b),
  color_lines = station_colors[2],
  color_imputations = "red",
  ylab = paste("PM10 ", "(", pollutant_units[["PM10"]], ")", sep = ""),
...
imputeTS::ggplot_na_imputations(
  tmp_ts_s, ts_s,
  title = paste(aq_station_names_short[3], "-", "outliers detection"),
  x_axis_labels = time(ts_s),
  color_lines = station_colors[3],
  color_imputations = "red",
  ylab = paste("PM10 ", "(", pollutant_units[["PM10"]], ")", sep = ""),
...
```{r}
plot_filtered_ts(
  original_ts = tsbox::ts_ts(ts_m),
  filtered_ts_list = list(simple_ma(tsbox::ts_ts(ts_m), p = 4), simple_ma(tsbox::ts_ts(ts_m), p = 8), simple_ma(tsbox::ts_ts(ts_m), p = 12)),
  main = paste(aq_station_names_short[1], "-", "simple moving average filter"),
  ylab = paste("PM10 ", "(", pollutant_units[["PM10"]], ")", sep = ""),
...
# plot_filtered_ts(
#  original_ts = ts_b,
#  filtered_ts_list = list(simple_ma(ts_b, p = 4), simple_ma(ts_b, p = 8), simple_ma(ts_b, p = 12)),
#  main = paste(aq_station_names_short[2], "-", "simple moving average filter"),
#  ylab = paste("PM10 ", "(", pollutant_units[["PM10"]], ")", sep = ""),
#  legend_names = c("p = 4", "p = 8", "p = 12"),
#  line_colors = filtering_colors
# )
# plot_filtered_ts(
#  original_ts = ts_s,
#  filtered_ts_list = list(simple_ma(ts_s, p = 4), simple_ma(ts_s, p = 8), simple_ma(ts_s, p = 12)),
#  main = paste(aq_station_names_short[3], "-", "simple moving average filter"),
#  ylab = paste("PM10 ", "(", pollut_units[["PM10"]], ")", sep = ""),
#  legend_names = c("p = 4", "p = 8", "p = 12"),
#  line_colors = filtering_colors
# )
```
...
```{r}
stl_ts_m <- ts_m %>% stl(s.window = 13, robust = TRUE)
stl_ts_b <- ts_b %>% stl(s.window = 13, robust = TRUE)
stl_ts_s <- ts_s %>% stl(s.window = 13, robust = TRUE)

plot(
  stl_ts_m,
  main = paste(aq_station_names_short[1], "-", "STL decomposition"),
  ylab = paste("PM10 ", "(", pollutant_units[["PM10"]], ")", sep = "")
)
plot(
  stl_ts_b,
  main = paste(aq_station_names_short[2], "-", "STL decomposition"),
  ylab = paste("PM10 ", "(", pollutant_units[["PM10"]], ")", sep = "")
)
plot(
  stl_ts_s,
  main = paste(aq_station_names_short[3], "-", "STL decomposition"),
  ylab = paste("PM10 ", "(", pollutant_units[["PM10"]], ")", sep = "")
)

Box.test(stl_ts_m$time.series[, "remainder"], lag = log(length(ts_m)), type = "Ljung-Box")
Box.test(stl_ts_b$time.series[, "remainder"], lag = log(length(ts_b)), type = "Ljung-Box")
Box.test(stl_ts_s$time.series[, "remainder"], lag = log(length(ts_s)), type = "Ljung-Box")
```

```{r}
plot_filtered_ts(
  original_ts = ts_m,
  filtered_ts_list = list(seasadj(stl_ts_m)),
  main = paste(aq_station_names_short[1]),
  ylab = paste("PM10 ", "(", pollut_units[["PM10"]], ")", sep = ""),
...
plot_filtered_ts(
  original_ts = ts_b,
  filtered_ts_list = list(seasadj(stl_ts_b)),
  main = paste(aq_station_names_short[2]),
  ylab = paste("PM10 ", "(", pollut_units[["PM10"]], ")", sep = ""),
  legend_names = "Seasonally adjusted",
  line_colors = filtering_colors[1]
)
plot_filtered_ts(
  original_ts = ts_s,
  filtered_ts_list = list(seasadj(stl_ts_s)),
  main = paste(aq_station_names_short[3]),
  ylab = paste("PM10 ", "(", pollut_units[["PM10"]], ")", sep = ""),
  legend_names = "Seasonally adjusted",
  line_colors = filtering_colors[1]
)
...
```{r}
plot_stl_components(
  list(stl_ts_m, stl_ts_b, stl_ts_s),
  main = "Trend components for PM10",
  component_name = "trend",
  ylab = paste("PM10 ", "(", pollut_units[["PM10"]], ")", sep = ""),
  names = aq_station_names_short
)
plot_stl_components(
  list(stl_ts_m, stl_ts_b, stl_ts_s),
  main = "Seasonal components for PM10",
