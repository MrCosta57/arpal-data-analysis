...
Data are daily but will be aggregated by month to perform a long term analysis and understand better the components. Later model on daily data will be developed to forecast the air quality in the region.
```{r}
# Pre-requirements
...
Data are aggregated by month so a long term analysis can be performed, as said. Also by aggregation in months, daily NA values are neglected
```{r}
...
par(mfrow = c(1, 2))
plot(
  Acf(ts_m_monthly, lag.max = as.integer(length(ts_m_monthly) / 4), plot = FALSE),
  main = paste(aq_station_names_code[1], "-", "ACF")
)
plot(
  Pacf(ts_m_monthly, lag.max = as.integer(length(ts_m_monthly) / 4), plot = FALSE),
  main = paste(aq_station_names_code[1], "-", "PACF")
)
par(mfrow = c(1, 1))
```
```{r}
par(mfrow = c(1, 2))
plot(
  Acf(ts_b_monthly, lag.max = as.integer(length(ts_b_monthly) / 4), plot = FALSE),
  main = paste(aq_station_names_code[2], "-", "ACF")
)
plot(
  Pacf(ts_b_monthly, lag.max = as.integer(length(ts_b_monthly) / 4), plot = FALSE),
  main = paste(aq_station_names_code[2], "-", "PACF")
)
```
```{r}
par(mfrow = c(1, 2))
plot(
  Acf(ts_s_monthly, lag.max = as.integer(length(ts_s_monthly) / 4), plot = FALSE),
  main = paste(aq_station_names_code[3], "-", "ACF")
)
plot(
  Pacf(ts_s_monthly, lag.max = as.integer(length(ts_s_monthly) / 4), plot = FALSE),
  main = paste(aq_station_names_code[3], "-", "PACF")
)
```
