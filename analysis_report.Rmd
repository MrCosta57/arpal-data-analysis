...
The Ljung-Box test supports the validity of the decomposition, as it does not suggest rejecting the null hypothesis that the residuals are white noise for all stations, except for station 571.
...
plot_diff_arima_acf_pacf(ts_m_train, aq_station_names_code[1], lag = as.integer(daily_ts_freq))
```
...
# plot_log_arima_acf_pacf(ts_b_train, aq_station_names_code[2], lag_max = 40)
```
```{r}
plot_diff_arima_acf_pacf(ts_b_train, aq_station_names_code[2], lag_max = 40)
# plot_diff_arima_acf_pacf(ts_b_train, aq_station_names_code[2], lag_max = 40, lag = 7)
# plot_diff_arima_acf_pacf(ts_b_train, aq_station_names_code[2], lag = as.integer(daily_ts_freq))
```
```{r}
...
# plot_log_arima_acf_pacf(ts_s_train, aq_station_names_code[3], lag_max = 40)
```
```{r}
plot_diff_arima_acf_pacf(ts_s_train, aq_station_names_code[3], lag_max = 40)
# plot_diff_arima_acf_pacf(ts_s_train, aq_station_names_code[3], lag_max = 40, lag = 7)
# plot_diff_arima_acf_pacf(ts_s_train, aq_station_names_code[3], lag = as.integer(daily_ts_freq))
```
```{r}
...
An example of train-test split of Milan's station data is shown below.
If dynamic regression the mean of the observation in the training set for the predictor is used
...
```{r}
largest_horizon <- 31
ts_m_final_model_list <- list(ma5_ts_m, auto_ts_m, nn_ts_m_auto, xreg_no2_ts_m)
ts_b_final_model_list <- list(ar1_ma2_ts_b, auto_ts_b, nn_ts_b_auto, xreg_no2_ts_b)
ts_s_final_model_list <- list(ar1_ma3_ts_s, auto_ts_s, nn_ts_s_auto, xreg_no2_ts_s)

IC_values_ts_m <- compute_arima_IC(arima_models = ts_m_final_model_list[-3], suffixes = list("", "[auto]", "[auto]"))
IC_values_ts_b <- compute_arima_IC(arima_models = ts_b_final_model_list[-3], suffixes = list("", "[auto]", "[auto]"))
IC_values_ts_s <- compute_arima_IC(arima_models = ts_s_final_model_list[-3], suffixes = list("", "[auto]", "[auto]"))
...
For conveniance forecast analysis is applied only to Station 548 in Milan but the same procedure can be applied to the other stations.
```{r}
...
best_arima_ts_m <- ts_m_final_model_list[-3][[best_idx]]
...
For modelling daily data probably are needed some more advanced models
or model them with model the seasonality using Fourier terms ^[https://robjhyndman.com/hyndsight/longseasonality/]
```{r}
